...
class ReplicaDataset:
    ...
    def infer_camera_key(self, rgb_path: str) -> Optional[str]:
        ...
        if self._cameras_by_key:
            for key in sorted(self._cameras_by_key.keys(), key=len, reverse=True):
                if f"{key}_" in name:
                    return key
    ...
    def _get_data_list_from_traj_imgname(
        ...
        for fi in range(n):
            ...
            if self._output_indexing == "timestamp":
                ...
                # xgrid 等：traj 已是完整 tag（如 camera_0_1769607848985263_0），与 results 同名
                direct_rgb = results_dir / f"{rgb_pfx}_{stem}{ext}"
                direct_dep = results_dir / f"{dep_pfx}_{stem}.png"
                if direct_rgb.is_file() and direct_dep.is_file():
                    found = (str(direct_rgb), str(direct_dep))
                if found is None:
