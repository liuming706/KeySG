...
def system_instruction_grounded_description() -> str:
    ...
    return (
        ...
        "each with a unique ID and semantic label. Some nodes may also include a 2D bounding box "
        "bbox_2d=[x1,y1,x2,y2] in pixel coordinates on the original, unannotated image.\n\n"
        ...
        "  • 'name': refined common name for the object (correct the label if imprecise). If bbox_2d is provided, inspect the original image content inside that box first and use the provided label only as a hint.\n"
        ...
        "- When bbox_2d is available, ground the object's identity, attributes, and state primarily in the original pixels inside the box; do not require drawn boxes or labels.\n"
